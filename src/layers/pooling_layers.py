...
class GatedPool2d(torch.nn.Module):
    ...
    def __init__(self, kernel_size, in_channels=None, stride=None, padding=0, dilation=1, ceil_mode=False):  #, normalization=None, denormalize=True):
        ...
        self.ceil_mode = ceil_mode
        if in_channels is None:
            self.coefficients = torch.nn.Parameter(torch.rand(kernel_size[0] * kernel_size[1]))
        else:
            self.coefficients = torch.nn.Parameter(torch.rand(1, in_channels, 1, 1, kernel_size[0] * kernel_size[1]))
    ...
    def forward(self, tensor):
        ...
        alpha_normalized = torch.sigmoid(torch.sum(tensor * self.coefficients, dim=-1, keepdim=False))
