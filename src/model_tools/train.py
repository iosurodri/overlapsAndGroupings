...
def train(name, model, optimizer, criterion, train_loader, scheduler=None, train_proportion=1, batch_size=128,
    ...
    # If we received a single DataLoader, we add it to an iterator, to comply with the logic of the rest of the training
    # process:
    if not isinstance(train_loader, collections.Iterable):
    ...
    for epoch in tqdm(range(num_epochs), unit='epochs'):
        for loader in train_loader:
            ...
            for i, data in enumerate(tqdm(loader, unit='batches', leave=False), 0):
