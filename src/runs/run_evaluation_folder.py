...
if __name__ == '__main__':
    ...
    for base_model_file_name in tqdm(os.listdir(os.path.join(PATH_MODELS, base_folder)), unit='model'):
        num_tests = len(list(filter(lambda x: not x.endswith('.json'), os.listdir(os.path.join(PATH_MODELS, base_folder, base_model_file_name)))))  # Count the number of files not corresponding to info files (.json)
        for test_idx in range(num_tests):
            try:
                ...
                print('Error in model: {}'.format(model_file_name))